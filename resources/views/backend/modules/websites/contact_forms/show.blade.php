    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="body">
                <div class="row">
                <div class="col-lg-6">
                    @if(dsld_check_permission(['add-contactfs']))
...
@section('footer')
    @include('backend.modules.websites.contact_forms.add')
    ...
    <div class="modal fade" id="edit_larger_modals" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="update_form" action="{{ route('contact_form.update') }}" method="POST" enctype="multipart/form-data" >
                @csrf 
...
<script>
    function add_new_lg_modal_form(){
        ...
        $('#add_new_larger_modals_tile').text('Add New Form');
    }
    ...
    function edit_lg_modal_form(id){
        ...
        $('#edit_larger_modals_title').text('Edit Form');

        $.ajax({
            url: "{{ route('contact_form.edit') }}",
            type: "post",
            cache : false,
            data: {
                    '_token':'{{ csrf_token() }}',
        ...
        event.preventDefault();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'send_mail': $('#edit_send_mail').val(),
                    'send_mail2': $('#edit_send_mail2').val(),
                    'admin_template': $('#edit_admin_template').val(),
                    'user_template': $('#edit_user_template').val(),
                    'redirect': $('#edit_redirect').val(),
                    'page_id': $('#edit_page_id').val(),
                    'success_msg': $('#edit_success_msg').val(),
                    'user_mail_subject': $('#edit_user_mail_subject').val(),
                    'mail_body': $('#edit_mail_body').val(),
                    'from_mail_id': $('#edit_from_mail_id').val(),
                    'layout': $('#edit_layout').val(),
