...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                @if(dsld_check_permission(['edit-page-others']))
                <th width="10">Status</th>
                @endif

                @if(dsld_check_permission(['edit-page-others']) || dsld_check_permission(['delete-page-others']))
        ...
        <tfoot>
            <tr class="text-center">
                ...
                @if(dsld_check_permission(['delete-page-others']) || dsld_check_permission(['edit-page-others']))
                <th width="15">Action</th>
                @endif
            </tr>
        </tfoot>
        ...
        @if(is_array($data) || count($data) > 0 )
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <td width="60">
                       Name: <a @if(empty($value->content)) # @else href="{{ route('custom-pages.show_custom_page', [$value->slug]) }}" @endif target="_blank"><strong>{{ $value->title }}</strong></a><br>
                    ...
                    <td width="15">
                            @if(dsld_check_permission(['edit-page-others']))
                            <a href="javascript:void(0)"  onclick="edit_lg_modal_form({{ $value->id }}, '{{ route('page.others.edit') }}', 'Category');" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            @if(dsld_check_permission(['delete-page-others']))
