...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                @if(dsld_check_permission(['edit-news-events']))
                <th width="10">Status</th>
                @endif

                @if(dsld_check_permission(['edit-news-events']) || dsld_check_permission(['delete-news-events']))
        ...
        <tfoot>
            <tr class="text-center">
                ...
                @if(dsld_check_permission(['delete-news-events']) || dsld_check_permission(['edit-news-events']))
                <th width="15">Action</th>
                @endif
            </tr>
        </tfoot>
        ...
        @if(is_array($data) || count($data) > 0 )
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <td width="15">
                            @if(dsld_check_permission(['edit-news-events']))
                            <a href="javascript:void(0)"  onclick="edit_lg_modal_form({{ $value->id }}, '{{ route('news.events.edit') }}', 'Category');" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            @if(dsld_check_permission(['delete-news-events']))
