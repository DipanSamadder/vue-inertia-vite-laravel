...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                <th>Title</th>
                <th>Date</th>
                @if(dsld_have_user_permission('galleries_edit') == 1)
                <th>Status</th>
                @endif
                <th>Action</th>
            </tr>
    ...
    <tbody>
        @if(is_array($data) || count($data) > 0 )
            ...
            @php $key++; @endphp
                <tr @if($value->level == 1)  class="table-success" @elseif($value->level == 2) class="table-info" @elseif($value->level == 0) class="table-danger" @endif>
                    <th scope="row">{{ $key }}</th>
                    <td>
                    ...
                    <td>
                        <p class="text-center mb-0 action_items">
                            ...
                            @if(dsld_have_user_permission('galleries_edit') == 1)
                            <a href="{{ route('galleries.edit', [$value->id]) }}"  class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            
                            @if(dsld_have_user_permission('galleries_delete') == 1)
