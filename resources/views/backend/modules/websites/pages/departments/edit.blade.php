 ...
 <form id="update_form" action="{{ route('pages.update') }}" method="POST" enctype="multipart/form-data" >
    ...
    <div class="row clearfix">
        ...
        <div class="col-lg-8">
            ...
            <input type="hidden" name="type" id="type" value="department_details" />
            ...
            <div class="card mb-0">
                ...
                <div class="form-group">                                
                    ...
                    <textarea class="summernote" id="content" onchange="is_edited()"><?php $str = $data->getTranslation('content', $lang); echo htmlspecialchars_decode($str); ?></textarea>                                   
                ...
                <div class="body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="template" id="template" onchange="is_edited()">
                            ...
                            <option value="departments_default" @if($data->template == 'departments_default') selected @endif>Default</option>                          

                            <option value="departments_abouts" @if($data->template == 'departments_abouts') selected @endif>About</option>                             

                            <option value="departments_message" @if($data->template == 'departments_message') selected @endif>Message</option>                             

                            <option value="departments_outcome" @if($data->template == 'departments_outcome') selected @endif>Outcome</option>                            

                            <option value="departments_offered" @if($data->template == 'departments_offered') selected @endif>Offered</option>                              

                            <option value="departments_listing" @if($data->template == 'departments_listing') selected @endif>Listing</option>                            

                            <option value="departments_details" @if($data->template == 'departments_details') selected @endif>Details</option>                            

                            <option value="departments_parents" @if($data->template == 'departments_parents') selected @endif>Parent</option>                            
                        ...
                        <select class="form-control show-tick ms select2" name="parent" id="parent" onchange="is_edited()">
                            ...
                            <option value="0">-- Please select --</option>
                                ...
                                @php 
                                    ...
                                    $childs  = App\Models\Post::where('status', 1);



                                    if($data->id == '72'){

                                        $childs = $childs->Where('type', 'custom_page');

                                    }else{

                                        $childs = $childs->where('type', 'department_details');

                                    }

                                    

                                    $childs = $childs->whereNotIn('id', [$data->id])->get();
                                ...
                                <option value="{{ $value->id }}"  @if($data->parent ==  $value->id) selected @endif>{{ $value->title }} ({{ $value->level }})</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control show-tick ms select2" name="cat_type" id="cat_type" onchange="is_edited()">

                            <option value="0">-- Please select --</option>



                            @foreach(App\Models\Post::where('type', 'programs_details')->whereNotIn('id', [73])->where('status', 1)->get() as $key => $value)

                                <option value="{{ $value->id }}"  @if($data->cat_type ==  $value->id) selected @endif>{{ $value->title }}</option>
            ...
            <div class="body">
                ...
                <ul class="nav nav-tabs p-0 mb-3 nav-tabs-success" role="tablist">
                    ...
                    @php 
                        ...
                        if($data->template == 'departments_abouts' && $sec->id == 14) { 

                            $active_tab = 'active';

                        }else if ($data->template == 'departments_message' && $sec->id == 15) { 

                            $active_tab = 'active';

                        }else if ($data->template == 'departments_outcome' && $sec->id == 17) { 

                            $active_tab = 'active'; 

                        }else if ($data->template == 'departments_offered' && $sec->id == 16) { 

                            $active_tab = 'active';

                        }else if ($data->template == 'departments_default' && $sec->id == 18) { 
                    ...
                    @endphp
                        ...
                        <li class="nav-item"><a class="nav-link {{ $active_tab }}" id="page_tab-{{ $sec->id }}" data-toggle="tab" href="#page_section-{{ $sec->id }}" onclick="edit_pages_section('{{ $sec->id }}', '{{ $data->id }}', '{{ $lang }}')"> {{ $sec->title }} </a></li>
                ...
                <div class="tab-content">
                    ...
                    <div role="tabpanel" class="tab-pane {{ $active_tab }}" id="page_section-{{ $sec->id }}">
...
<style type="text/css">
    ...
    /** .note-editing-area img{max-width: 100% !important} **/
...
<script>
    ...
    function edit_pages_section(section_id, page_id, lang){
        ...
        $.ajax({
            ...
            url: "{{ route('departments.edit_extra') }}",
