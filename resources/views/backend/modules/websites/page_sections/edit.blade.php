        ...
        <div class="col-sm-4">
            <div class="form-group">
                ...
                <select class="form-control show-tick ms select2" name="page_id" id="edit_page_id" onchange="is_edited()">
                    ...
                    @foreach(App\Models\Post::orderBy('title', 'ASC')->where('status', '1')->get() as $key => $value)
                        <option value="{{ $value->id }}" @if($data->pageable_id == $value->id) selected @endif>{{ $value->title}}- ({{ $value->id }})</option>
                    @endforeach
                </select>                                                                
            </div>
        </div>
